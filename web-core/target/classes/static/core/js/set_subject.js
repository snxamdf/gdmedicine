						...
						$.ajax({
							...
							success: function(res){
								...
								if(res.status == 200){
									...
									window.location.href="/subj/subject/setForSubj"+backUrl;
								}
...
//					url: 'php/init_subject_del.php',
	...
	//切换显示、隐藏
	$(".subjectSwitch").live('click', function(e){
		e.preventDefault();
		var subjectId = $(this).closest("tr").find('input[name="subjectid"]').val();
		var isShow = $(this).closest("tr").find('input[name="isShow"]').val();
		var _this = $(this);
		//th:href="@{${#ctl.path}+'/subj/subject/stopSubj'(subjectId=${domain.id},isShow=0)}"
		$.ajax({
			type: 'POST',
			url: ctl.path +'/subj/subject/switchSubjShow',
			data: {
				"subjectId":subjectId,
				"isShow":isShow,
				"_csrf" : $("input[name='_csrf']").val()
