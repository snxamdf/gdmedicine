...
public class TokenInterceptor extends HandlerInterceptorAdapter {
			...
			if (annotation != null) {
				...
				if (isRemoveSession) {
					if (!Tokens.valid(request)) {
						...
						logger.error("{_token:'" + token
								+ "',error:'重复提交错误',url:'" + url
								+ "',sessionId:'" + sessionId + "'}");
						String requestType = request
								.getHeader("X-Requested-With");
						if (requestType != null
								&& "XMLHttpRequest".equals(requestType)) { // ajax请求
							PrintWriter out = response.getWriter();
							out.print(Results.fault(403, "token error", "")
									.toJson());
	...
	public void afterCompletion(HttpServletRequest request,
			HttpServletResponse response, Object obj, Exception mav)
			throws Exception {
	}
